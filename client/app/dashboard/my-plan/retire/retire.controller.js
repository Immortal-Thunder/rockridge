...
.controller('RetireViewCtrl', function($scope, Auth, User) {
  ...
  //
  var endSavings = $scope.plan.retireProjection.retireProj.endSavings;
  var shouldSave = endSavings[2];
  var endSavingsMin = $scope.plan.retireProjection.retireProj.endSavings;
  var minSavings = endSavingsMin[2];

  console.log('shouldSave', shouldSave);
  console.log('minSavings', minSavings);
...
var getRetireProjDataArray = function(group){
  ...
  for(var key in group){
    ...
    interest.push(group[key].interest);
  }
...
var getRetireProjDataArray2 = function(group){
  var needs = [];
  
  for(var key in group){
    needs.push(group[key].needsWithInflation);
  }
...
/// call func here
var valuesArray = [];
valuesArray = getRetireProjDataArray($scope.plan.retireProjection.retireProj);
var needs = getRetireProjDataArray2($scope.plan.retireProjection);
console.log($scope.plan.retireProjection.needsWithInflation);
// for(var i=0; i<valuesArray[0].length; i++){
//   console.log(valuesArray[0][i]);
// }



$('#retireProjFixedContainer3').highcharts({
    chart: {
            zoomType: 'x'
        },
        title: {
            text: 'Total Savings with a Fixed Rate 7% and 3% Inflation'
        },
        subtitle: {
